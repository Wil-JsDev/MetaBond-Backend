...
public class RoleRepository(MetaBondContext metaBondContext) : IRoleRepository
{
    ...
    {
       return await metaBondContext.Set<Roles>().FirstOrDefaultAsync(r => r.Id == rolId, cancellationToken: cancellationToken);
    }

    public async Task<Roles?> GetByNameAsync(string roleName, CancellationToken cancellationToken)
    {
        return await metaBondContext.Set<Roles>().FirstOrDefaultAsync(r => r.Name == roleName, cancellationToken: cancellationToken);
    }
}