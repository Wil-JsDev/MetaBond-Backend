    ...
    ) : ICommandHandler<ConfirmAccountCommand, string>
{
        ...
        )
    {
        ...
        if (!result.IsSuccess)
        {
            logger.LogError("ConfirmAccountCommand: Failed to confirm account for user '{UserId}' using code '{Code}'. Reason: {ErrorMessage}", 
                userId, request.Code, result.Error);

            return ResultT<string>.Failure(result.Error!);
        }

        logger.LogInformation("ConfirmAccountCommand: Account confirmation successful for user '{UserId}' with code '{Code}'.", 
            userId, request.Code);
